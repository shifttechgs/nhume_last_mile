            ...
            <div class="space-y-4" style="flex:0 0 300px;position:sticky;top:88px;align-self:flex-start">
                ...
                <div style="background:#fff;border:1px solid #E9EAEC;border-radius:16px;overflow:hidden;">
                    ...
                    <div class="px-5 py-5 space-y-3">
                        {{-- Route preview --}}
                        <div>
                            <p class="text-sm font-bold text-gray-800 leading-tight" x-text="route_label || 'Harare → Bulawayo'"></p>
                        </div>
                        {{-- Departs preview --}}
                        ...
                        <div style="border-top:1px solid #F0F1F0;padding-top:12px;" class="space-y-2">
                            {{-- Avatar + badge --}}
                            @php $tp = Auth::user()->transporterProfile; $tier = $tp?->trust_tier?->value ?? 'unverified'; @endphp
                            <div class="flex items-center gap-2">
                                ...
                                <div>
                                    ...
                                    @endphp
                                    <span class="text-[10px] font-bold px-1.5 py-0.5 rounded" style="background:{{ $tbg }};color:{{ $tc }}">{{ $tl }}</span>
                                </div>
                            </div>
                            {{-- Chips --}}
                            <div class="flex gap-1.5 flex-wrap">
                                <span class="text-[11px] text-gray-500 border border-gray-200 rounded px-2 py-0.5" x-text="weight ? weight + ' kg space' : '— kg space'"></span>
                                <span class="text-[11px] text-gray-500 border border-gray-200 rounded px-2 py-0.5" x-text="(slots || '1') + ' slot' + ((slots || '1') != '1' ? 's' : '')"></span>
                            </div>
                            {{-- Notes preview --}}
                            <p class="text-[11px] text-gray-400 italic leading-relaxed" x-show="notes" x-text="'\"' + notes.slice(0,80) + (notes.length > 80 ? '…' : '') + '\"'"></p>
                        </div>
                        {{-- Price + WhatsApp --}}
                        <div style="border-top:1px solid #F0F1F0;padding-top:12px;" class="flex items-center justify-between">
                            <p class="text-sm font-bold text-gray-800" x-text="price ? '$' + parseFloat(price).toFixed(2) + ' / kg' : 'Negotiate'"></p>
                            <span class="inline-flex items-center gap-1 px-2.5 py-1.5 rounded-lg text-[11px] font-bold text-white" style="background:#25D366;">
